...
- 进度：11/16 个 G2 任务、41/101 个总任务完成
- 风险：`LIST-007` 本地完整质量、真实 PostgreSQL、升级、运行时和桌面/移动 E2E 已通过，等待受保护托管门禁；Windows 本机无 Redis/ClamAV/Docker，真实服务集成与四镜像 smoke 由 GitHub Linux 门禁执行；生产招聘/禁售品/专业服务政策、抽检比例、SLA 和原因码仍需运营/法律确认，v3 规则只把字段级风险证据送人工队列而不自动处罚
...
- `ORG-002` 保留原 G1/P1 标签并已由 PR #25 受保护合并；`LIST-006` 已由 PR #26 / final main run
  `30443279395` 受保护合并；现按 `IMPLEMENTATION_SEQUENCE.md` 执行 `LIST-007`。`MEDIA-003` 仍属于
  G4 受限验证文件。
- 需要生产品牌域名与资产权属确认。
