...
Not run: Real Redis and ClamAV integrations were skipped because those local services are unavailable and LIST-002 does not consume either；local Docker image smoke is unavailable because the host has no Docker CLI；protected Linux/real-service/image checks are pending the task PR
