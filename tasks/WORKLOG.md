...
Known gaps: Protected CI and merge evidence remain required before marking `PERF-001` done；production CWV p75 and API p95/SLO claims require a representative Beta traffic window and real dashboards rather than local synthetic evidence；`SEO-002` still owns sitemap/schema.org、`SEO-004` owns full automated/manual accessibility evidence，and later `PERF-002` owns load/endurance/fault-injection validation
