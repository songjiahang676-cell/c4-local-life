...
Not run: Real PostgreSQL、Redis、OpenSearch and ClamAV integration suites plus four-image smoke are unavailable locally because Docker CLI and disposable service URLs are absent；the local full suite reports all 88 service skips instead of treating them as passes。Protected GitHub CI must supply fresh/upgrade PostgreSQL、real services、Linux Chromium and four non-root images before completion evidence is final
