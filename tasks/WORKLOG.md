...
Known gaps: `A11Y-001` Narrator/Edge speech and `A11Y-002` actual 200% browser zoom remain Gate 3 blockers；`SEO-004` stays todo and the Gate checklist stays unchecked until both have real results。Protected Linux real-service/E2E/four-image evidence and merge also remain pending。Future message/payment/video and complete document-language/i18n templates cannot inherit the current zero-violation claim and must enter their own task matrix
