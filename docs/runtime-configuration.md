...
| `OPENSEARCH_INDEX_PREFIX`                      | 否   | 否   | 版本化 Listing 索引前缀，默认 `socal_local`     |
| `SEARCH_QUERY_TIMEOUT_MS`                      | 否   | 否   | 单次搜索执行上限，默认 1500、最长 5000 ms       |
| `SEARCH_PIT_KEEP_ALIVE_SECONDS`                | 否   | 否   | 搜索分页 PIT/cursor 窗口，默认 120、最长 300 秒 |
