...
`30401011927` 合并为 `ca506c8`。当前 `AUTH-004` 已实现可选 scrypt 密码登录、通用防枚举失败、
账号/IP/设备限频与持久锁定、恢复冷却、只存哈希的单次 token、成功后全会话撤销和审计。真实
PostgreSQL 52 项、全仓 48 个文件共 179 项测试、8 个构建、运行时可观测性检查和 Chromium
桌面/移动 6/6 smoke 已通过；PR #14 / run `30402574230` 的托管质量和四应用非 root 容器检查也已
通过并待合并。真实短信/邮件通知适配器保留到 `NOTIF-001`/`EVT-001`，在适配器不可用时公开响应
保持一致且不泄露恢复 token。
