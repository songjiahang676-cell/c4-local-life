...
合并为 `9c66b87`。当前已完成 ORG-001 的硬依赖 `API-004`：PII 最小化 Actor/RequestContext、显式
Policy 注册、声明式控制器 Guard、默认拒绝和可复用对象授权矩阵；本地真实 PostgreSQL 套件 33/33、
全仓 27 个测试文件共 99 项测试及 8 个构建通过，受保护托管检查通过前不进入 ORG-001。
