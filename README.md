...
`8590060`）。Gate 1 随后从 `AUTH-001` 开始：本地已完成 HMAC-only 会话、原子旋转/注销、绝对与
闲置过期、请求认证上下文和安全 Cookie，并通过空库/升级迁移、真实 PostgreSQL 集成、22 个测试文件
共 66 项测试和全部构建；托管 PR 检查完成前不开始 `AUTH-002`。
