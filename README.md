...
PR #7 / run `30388093140` 的完整质量和四应用非 root 镜像检查合并为 `ab09c81`。当前 `TAX-001`
已实现中英 Region/Category 树、受控 alias 表与归一化查询、active-only 公开过滤和幂等 seed；
本地真实 PostgreSQL 套件 39/39、全仓 33 个测试文件共 121 项测试及 8 个构建通过，受保护托管检查
通过前不进入 TAX-002。
