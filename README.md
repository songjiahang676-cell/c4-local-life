...
PR #8 / run `30389838047` 合并为 `d622f74`。当前 `TAX-002` 已实现动态表单不可变版本、
乐观 draft/preview、原子 publish、追加式 rollback、历史 Listing 版本校验、严格配置安全约束和
公开缓存读取；本地真实 PostgreSQL 套件 42/42、全仓 36 个测试文件共 131 项测试及 8 个构建通过，
受保护托管检查通过前不进入下一任务。
